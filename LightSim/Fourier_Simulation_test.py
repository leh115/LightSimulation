...
class Test_Simulation(unittest.TestCase):
    ...
    def test_Forwards_Propagation(self):
        ...
        self.assertGreater(
            self.analyse.beam_width(propagator.Beam_Cross_Sections[-1]),
        ...
        propagator2.Beam_Cross_Sections = test_mode
        ...
        if z_distance> propagator.dz * 6: # gouy doesn't work for low propagation distances
            ...
            propagator >> z_distance
            plane_shift = np.angle(propagator.Beam_Cross_Sections[-1][int(propagator.Nx/2 - 1), int(propagator.Ny/2 - 1)]) # get the central value
            gauss_shift = np.angle(propagator2.Beam_Cross_Sections[-1][int(propagator2.Nx/2 - 1), int(propagator2.Ny/2 - 1)])
            
            sim_gouy_shift = plane_shift-gauss_shift
            gouy_phase_shift = -np.arctan(z_distance / propagator2.Rayleigh_Length(self.InitialBeamWaist))
            ...
            self.assertAlmostEqual(sim_gouy_shift, gouy_phase_shift, 1, msg=f"Gouy phase shift should be: {gouy_phase_shift}")
    ...
    def test_reproducible_propagation(self):
        ...
        self.assertAlmostEqual(
            self.analyse.beam_width(self.propagator1.Beam_Cross_Sections[-1]),
    ...
    def test_reproducible_planes_and_propagation(self):
        ...
        for _ in range(2):
            self.propagator1.Beam_Cross_Sections = self.test_mode1
            self.propagator1.Propagate_FromPlane_ToPlane(0, len(self.propagator1.PlaneSetUp))
            ...
            self.propagator2.Propagate_FromPlane_ToPlane(0, len(self.propagator1.PlaneSetUp))

            # * Test that for override dz a system with n planes has 2n + 1 cross sections + initial cross section
            self.assertEqual(len(self.propagator2.Beam_Cross_Sections), 2 * (len(self.propagator1.PlaneSetUp) - 1 ) + 2)

            self.propagator2.Beam_Cross_Sections = self.test_mode1
            self.propagator2.Propagate_FromPlane_ToPlane(0, len(self.propagator1.PlaneSetUp),Forwards = False)
    ...
    def test_update_intermediate_value_theorem(self):
        ...
        propagator1 = Propagate(show_beam=False) # must re-start the class for new class variables to take effect
    ...
    def test_multi_mode_interaction(self):
        ...
        for plane_angle, eim, ere in zip(plane_angles, expected_imaginary, expected_real):
            for i, number_of_modes, start_mode,  in zip([1,2,3], [1,2,2], [0,1,0]):
                ...
                self.assertAlmostEqual( np.imag(np.sum(Planes[-1][self.test_plane1] - multi_mode_factor*mask_offset)) , eim*multi_mode_factor, msg=f"Plane {str(i)} imaginary components not {eim*multi_mode_factor}, Phase Plane Angle: {np.angle(plane_angle[0]+1J*plane_angle[1])}")

                self.assertAlmostEqual( np.real(np.sum(Planes[-1][self.test_plane1] - multi_mode_factor*mask_offset)), ere*multi_mode_factor, msg=f"Plane {str(i)} real components not {ere*1*multi_mode_factor}, Phase Plane Angle: {np.angle(plane_angle[0]+1J*plane_angle[1])}")
